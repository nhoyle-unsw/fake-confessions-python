...
real_file_lines = real_file_contents.count('\n')
...
def main():
    ...
    realFileTokenised = append_marker_end_of_lines(real_file_contents)
    fakeFileTokenised = append_marker_end_of_lines(fake_file_contents)

    print("Real file tokenised:\n\nstart[" + realFileTokenised + "]end")
    print("Real file lines: ", real_file_line_count)
    print("Fake file tokenised:\n\nstart[" + fakeFileTokenised + "]end")
...
for file_version in range(2 ** real_file_lines - 1):
    ...
    for bit in reversed(binary_pattern):
        ...
        if index >= len(binary_pattern):
            break
    updated_file_content = "\n".join(split_lines)
    print("updated file verison#:", file_version,
          "binary_pattern:", binary_pattern, "\n")
    print("File content: start[" + updated_file_content + "]end")
    input("waiting outer...\n\n")

# places a token at the end of the line


def append_marker_end_of_lines(string_contents):
    # replaced_string_contents = re.sub(r'\n', ' X\n', string_contents)
    replaced_string_contents = re.sub(r'\n', ' \n', string_contents)
    return replaced_string_contents
